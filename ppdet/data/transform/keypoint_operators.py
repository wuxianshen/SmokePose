...
class TopDownAffine(object):
    ...
    def __call__(self, records):
        ...
        for joint in joints:
            if len([x for x in joint if x < -1E-4]) > 0:
                print(f'---- {records["image_file"]} joints: {joints} ---- ')
...
class ToHeatmapsTopDown(object):
    ...
    def __call__(self, records):
        ...
        if 0 in records['target_weight']:
            print(f'---- Invalid target_weight: '
                  f'{records["image_file"]}: {records["target_weight"]} ----')
...
class ToHeatmapsTopDown_DARK(object):
    ...
    def __call__(self, records):
        ...
        for joint_id in range(num_joints):
            ...
            # Check that any part of the gaussian is in-bounds
            if math.isnan(mu_x) or math.isnan(mu_y) \
                    or math.isnan(tmp_size):
                print(records)
                print(joints)
                print(feat_stride)
            ul = [int(mu_x - tmp_size), int(mu_y - tmp_size)]
