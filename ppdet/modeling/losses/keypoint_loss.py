...
class AELoss(object):
    ...
    def apply_single(self, pred, tagmap):
        # Support num_joints=1
        # tagmap shape: (max_people, num_joints, 4)
        while len(tagmap.shape) < 3:
            tagmap = paddle.unsqueeze(tagmap, 0)
        # pred shape: (num_joints, H, W)
        while len(pred.shape) < 3:
            pred = paddle.unsqueeze(pred, 0)
