...
from ppdet.task_config import update_config, _C

from ppdet.utils.logger import setup_logger
logger = setup_logger('eval')


def parse_args(in_script_config=None):
    parser = ArgsParser(in_script_config)
    parser.add_argument(
        "--output_eval",
        default=None,
        type=str,
...
def main():
    config = None
    #config = '..\\local_data\\yml\\BottomUp\\higherhrnet_hrnet_w32_512_skp_cf_source.yml'
    #config = '..\\local_data\\yml\\TopDown\\hrnet.yml'
    config = '..\\local_data\\yml\\BottomUp\\SmokePose.yml'

    FLAGS = parse_args(config)
    FLAGS.eval = True

    FLAGS.task_cfg = '../local_data/task_yml/skp.yml'
    update_config(_C, FLAGS)
